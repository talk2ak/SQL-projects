...
from expenses
group by category;	-- this will not work for all rows. group by will show one output row per group   
-- what happened in above query is it grouped amount column where there are 12 records. and in result only two rows are identical(2700).
-- so group by aggregated the result set into 11 rows. 

# examples of some group by-
select category,
	sum(amount)
		-- amount*100/sum(amount) as pct
from expenses
...
select *,
		amount*100/sum(amount) over() as pct 	-- if nothing is specified inside over() function, it will take all the rows as one window
from expenses
order by category;
...
select *,
		amount*100/sum(amount) over(partition by category) as pct  -- to specify the set of rows by category we use partition by clause in over clause
from expenses
order by category;
